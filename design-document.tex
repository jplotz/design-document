...
\subsection{How Supervisord Manages Processes Inside Each Docker Compose Container}

Docker Compose controls containers, but it does not manage the individual processes running inside them. Each service in the \texttt{docker-compose.yml} file starts exactly one container, and Docker expects that container to run a single main foreground process. Many real applications, however, need several processes running at the same time. Examples include a web server, a background worker, and a scheduler. Docker cannot manage these processes by itself.

Supervisord solves this problem by acting as a process manager \emph{inside} the container. When the container starts, Supervisord becomes the main process (PID~1). It then reads its configuration files and starts all required internal programs. Each program is defined with a command, restart policy, and logging settings. Supervisord keeps every process running for the entire lifetime of the container.

If any internal process crashes, Supervisord automatically restarts it. This prevents the container from exiting unexpectedly and keeps the service stable. Supervisord also collects the output of all processes into log files, making it easier to understand what is happening inside the container.

When Docker Compose stops the container, Supervisord receives the stop signal and forwards it to all managed processes. This allows each process to shut down cleanly instead of being terminated abruptly.

In summary, Docker Compose manages containers, while Supervisord manages the processes inside each container. This combination allows a single container to run multiple long-lived processes reliably, while still fitting cleanly into a multi-container Compose environment.
