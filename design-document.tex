...
\begin{figure}[H]
    \centering
    \includegraphics[keepaspectratio,height=0.5\textheight,width=\textwidth]{AD-system-overview.png}
...
Django's MVC architecture and template system enforces clear separation of concerns and makes prototyping fast and easy. Combined with its powerful ORM, simple views can be implemented in a few lines of code (\ref{fig:indexview}).
\begin{figure}[H]
	\centering
	\begin{minted}{python}
	def index(request):
...
Finally, the team could also automate image rebuilding and vulnerability patching. A nightly rebuild process could use docker.build() to refresh base images, integrate new security updates, and re-validate application configurations. Combined with continuous secret scanning and monitoring, these improvements will make WADT more robust and sustainable as an educational security platform.
...
cVWA (Custom Vulnerable Web Application) is a lightweight, intentionally insecure training application designed to demonstrate common web vulnerabilities in a controlled environment. It typically runs on a simple PHP and MySQL stack, making it easy to deploy in Docker or local LAMP setups. The application exposes multiple endpoints with deliberately weak validation, insecure session handling, and flawed database interactions.

\paragraph{Educational value}
cVWA provides hands-on practice with classic web vulnerabilities such as SQL injection, XSS, insecure direct object references, and weak authentication flows. Because the application is intentionally minimal, learners can focus on understanding the vulnerability mechanics without the complexity of a full framework. This makes cVWA ideal for introductory security labs, demonstrations, and rapid experimentation.

\paragraph{Deployment notes and Docker considerations}
cVWA is typically deployed using a simple Docker Compose configuration that launches a PHP/Apache container alongside a MySQL database. Environment variables must be aligned across the \texttt{docker-compose.yml} and application configuration files. Volumes can be used to persist the database state. Once deployed, the application is accessible at \url{http://localhost/cvwa/} or the configured virtual host.

\paragraph{Why chosen}
cVWA was selected because of its simplicity, predictable behavior, and suitability for teaching foundational web security concepts. Its minimal design makes it easy to integrate into WADT’s container-based lab environment while still offering meaningful, hands-on vulnerability exploration.
...
Postman's documentation overview includes resources describing all of the functionalities of the software \cite{postman-overview}. The functions that are notable for use in our project include:
\begin{itemize}
\item User-Friendly Interface: Postman has its own application with a straightforward interface, making it easy to quickly set up and execute API tests without extensive configuration.
...
We considered two tools for documenting our APIs, Hopscotch and SwaggerHub. Both provide a standardized way to describe REST APIs. However, they differ in cost, accessibility, and integration capabilities within our existing testing structure.
...
SwaggerHub offers additional advanced features such as version control, various collaboration tools, and even automatic documentation generation. It can also be integrated with CI/CD tools to keep API documentation in sync with live deployments. 
...
\begin{itemize}
	\item Response time. While the Docker Engine often takes a significant amount of time to complete requests, we must ensure that users experience a sub-200ms response time. Workarounds may be developed to provide preliminary responses before the Docker Engine completes its work, in an async fashion, so that the site isn't perceived as slow due to its dependency on Docker.
	\item Resource usage. Network usage is especially important to measure and minimize, since it is the most likely avenue for hosting bills to come from. Third-party requests sent by target vulnerable webapps must be monitored.
	\item Previous two metrics under concurrent user load. While the original scope of WADT allows for a single-user design, in all practicality it will be desirable to extend to multiple concurrent users in the future.
\end{itemize}
...
The WADT project is hosted on GitHub. The team will make use of GitHub Actions to automate deployment, testing, and security scanning.
The intended components are as follows:
\begin{itemize}
	\item TruffleHog (Security Scanning) step. Checkout the main branch and use TruffleHog to check for potential password or data leaks.
	\item Backend test step. Use libraries such as \texttt{pytest} with Django to run test suite and report coverage.
	\item Frontend test step. Report coverage.
	\item Build step for frontend and backend. Install dependencies and upload build artifacts before deployment.
	\item Deployment to AWS step. Run DB migrations, issue system commands, and give status update on deployment.
\end{itemize}
...
Since AWS hosting provides a full Linux VM for the project, under current scope restrictions, each database would be installed locally
and the bits and bytes of our data would exist on the same hosting as our site code and web server. The potential for growth of log entries
beyond available storage space is not considered during the testing and development phase. A later migration to an independent database service
with more storage capabilities may be completed if it is determined that a pileup log entries will grow beyond current limits under normal usage.
...
\subsection{Overview of Supervisord}

Supervisord is a simple and reliable process manager that is used to run and control multiple long-running processes. It acts like a lightweight init system: it starts programs, keeps them running, restarts them if they fail, and provides a single place to view logs. This makes it especially useful in situations where several processes must run together inside one environment.

In modern deployments, Supervisord is commonly used inside containers. Docker normally expects each container to run only one main process. However, many real applications need more than one process at the same time, such as a web server, a background worker, and a scheduler. Supervisord solves this problem by managing all of these processes inside a single container. It ensures that each process starts correctly, stays running, and is restarted automatically if it crashes.

Supervisord works by reading configuration files written in a simple INI-style format. These files define each program's command, environment, restart behavior, and logging settings. Typical configuration files are stored in \texttt{/etc/supervisor/supervisord.conf} and \texttt{/etc/supervisor/conf.d/}. Because the configuration is separate from the application code, it is easy to update, version-control, and reuse across different environments.

One of the main benefits of Supervisord is its unified logging system. All managed processes can have their output collected into log files, making it easier to debug issues. In containerized environments, these logs can be mounted to the host or sent to external logging systems. This provides consistent visibility into what each process is doing.

Supervisord also handles shutdowns cleanly. When the container receives a stop signal, Supervisord forwards that signal to all of its child processes. This allows each process to shut down properly instead of being terminated abruptly. This behavior helps prevent data loss, incomplete tasks, or corrupted files.

In multi-container web applications, Supervisord works together with tools like Docker Compose or Kubernetes. These tools manage containers, networks, and scaling, but they do not manage the internal processes inside each container. Supervisord fills this gap by controlling the processes inside the container while the orchestration tool controls the container itself. This separation keeps the system organized and predictable.

Supervisord also includes an optional XML-RPC interface that allows external tools to check the status of processes or restart them remotely. While not always needed in container setups, this feature can be useful in more advanced deployments.

In summary, Supervisord is a practical and dependable tool for managing multiple processes inside a single container or server. It provides automatic restarts, clean shutdowns, centralized logging, and simple configuration. When combined with container orchestration tools, it helps ensure that complex applications run smoothly and consistently across different environments.

\subsubsection{How Supervisord Manages Processes Inside a Docker Container}

Supervisord is a lightweight process control system that runs as the primary process inside a container and is responsible for launching and supervising all other internal processes. When the container starts, Supervisord reads its configuration file and initializes each defined program with its own command, environment variables, and logging rules. This allows a single container to run multiple services in a controlled and predictable manner.

Once active, Supervisord continuously monitors the health and status of each managed process. If a program exits unexpectedly or becomes unresponsive, Supervisord can automatically restart it according to the specified policy. This ensures that essential services such as web servers, background workers, or schedulers remain available throughout the container's lifetime. Startup priorities and dependency ordering can also be defined, allowing complex applications to initialize in a stable sequence.

Supervisord centralizes logging by capturing the standard output and error streams of each process and writing them to log files inside the container. This unified logging approach simplifies debugging and provides clear visibility into how different processes behave and interact. Administrators can also query Supervisord for process status, stop or restart individual programs, and adjust configurations without modifying the container's base image.

During shutdown, Supervisord receives the termination signal and gracefully forwards it to all managed processes. This coordinated shutdown sequence allows each service to close connections, flush buffers, and complete pending work before exiting. By acting as an internal init system, Supervisord ensures clean termination and reduces the risk of corrupted data or inconsistent application state.

Overall, Supervisord provides structured, reliable control over multi-process behavior inside a container. It launches programs, monitors their health, manages logs, restarts failures, and coordinates orderly shutdowns, making it a practical solution for scenarios where multiple internal processes must run together within a single container.

\textbf{Note:} For this project, we use Docker Compose instead of Supervisord because Supervisord requires produces a large amount of data that becomes difficult to manage at scale. It also becomes complex for us to fix onto the containers, which is why we're using Docker Compose.

\subsection{Docker Architecture}
% Docker team content follows
Docker provides a comprehensive containerization framework that underpins the way we deploy and manage intentionally vulnerable web applications. At its core, Docker simplifies the packaging of applications by bundling code, dependencies, and runtime settings into portable units that can run consistently across different environments. This consistency is crucial for security testing, where reproducibility and isolation are required to ensure that vulnerabilities can be explored without interference from host systems or other applications. By abstracting away platform differences and offering lightweight, isolated execution environments, Docker makes it possible to replicate complex setups quickly and reliably, whether on local machines, in labs, or in cloud platforms.

In practice, Docker serves as the foundation of our deployment pipeline, providing the tools and workflows needed to build, launch, monitor, and reset vulnerable applications with precision. It supports rapid iteration by enabling environments to be spun up and torn down in seconds, while also offering mechanisms for persistence, controlled networking, and resource management. These capabilities allow us to simulate realistic attack surfaces, enforce safe boundaries, and maintain clean resets between test cycles. With Docker as the architectural backbone, we can integrate containerized applications into broader testing frameworks, automate deployments, and ensure that every experiment begins with a secure, reproducible environment tailored for vulnerability research.

\subsubsection{Core Components}
The Docker architecture consists of these key elements:
  \begin{itemize}
    \item {Docker Engine} – The Docker Engine is the core runtime that builds, manages, and runs containers on the host system. It handles image creation, container execution, networking, and storage, acting as the backbone of all Docker operations.
    \item {Images} – Docker images are read-only templates that contain everything needed to run an application, including code, libraries, and environment settings. We build custom images using Dockerfiles and select lightweight, secure base images to minimize attack surface and improve performance.
    \item {Containers} – Containers are live, running instances of images that execute applications in isolated environments. They use Linux features like namespaces and cgroups to separate processes and control resource usage, making them ideal for safely running vulnerable apps.
    \item {Volumes} – Volumes provide persistent storage that exists outside the container’s writable layer, allowing data to survive restarts and removals. We use volumes to store logs, payloads, and forensic traces for analysis without modifying the container itself.
    \end{itemize}
...
We considered bootstrap due to the high synergy with react. 
...
Bellow is some code that shows how to make a button component using Bootstrap and REACT-Bootstrap.
\begin{verbatim}
...
Bootstrap simplifies the process of designing a website with REACT, allowing the developer to focus on functionality rather than aesthetic.


During research, one of our developers found custom bootstrap themes that would help offset the non-flexible nature of Bootstrap.
...
Instead of having pre-designed components like bootstrap, tailwind gives a lot of freedom to the developer to design their own components using CSS.
...
Here is an example of a button component using tailwind CSS in REACT:
\begin{verbatim}
...
Tailwind allows the developer more creative freedom when it comes to frontend designs.
However, as shown in the code above, it's learning curve is dependent on the developer's knowledge of CSS.
If we decide to use tailwind, our frontend team will have to dedicate extra time to understand CSS code, as well as tailwind's unique utility-first approach.
...
This was a nice way to look at what a full website using Chakra UI can do.
...
Here is an example of a button component using Chakra UI in REACT:
\begin{verbatim}
...
Chakra UI is similar to bootstrap in the sense that it provides pre-designed components for the developer to use. 

\subsubsection{Comparison Summary}
Each of the frontend CSS libraries we researched had their own pros and cons.
Here is a chart that summarizes our findings:
\begin{table}[H]
\centering
\begin{tabular}{|l|l|l|}
\hline
\textbf{CSS Library} & \textbf{Pros} & \textbf{
Cons} \\ \hline
Bootstrap &
\begin{tabular}[c]{@{}l@{}}- Popular and well-documented\\ - REACT-Bootstrap integration\\ - Child themes available\end{tabular} & \begin{tabular}[c]{@{}l@{}}- Less flexible design\\ - Requires CSS overrides for uniqueness\end{tabular} \\ \hline
Tailwind &
\begin{tabular}[c]{@{}l@{}}- Highly flexible and customizable
\\ - Utility-first approach\\ - Large community support\end{tabular} & \begin{tabular}[c]{@{}l@{}}- Steeper learning curve\\ - No official REACT library\\ - Third-party REACT library is paid\end{tabular} \\ \hline
Chakra UI &
\begin{tabular}[c]{@{}l@{}}- Developer-friendly\\ -
	 Good Figma integration\\ - Modern component design\end{tabular} & \begin{tabular}[c]{@{}l@{}}- Lacks some components\\ - Limited theming options\end{tabular} \\ \hline
\end{tabular}
\end{table}
...
From our findings, we narrowed down the decision to REACT-Bootstrap and Chakra UI, as both libraries provided a good developer experience.
Tailwind is a strong contender, allowing the developer more creative freedom with their component designs.

However, ultimately decided against due to is lack of an official REACT library, and steep learning curve.
We also felt that this website prioritized functionality over creative design, making the other options more suitable.
...
Chakra UI was a strong contender and was also a good option. 
Ultimately, the deciding factor was previous developer experience with Bootstrap, as both libraries provided a good developer experience,
and good dynamic components.

Our frontend team plans to compensate for Bootstrap's less flexible design by utilizing Bootswatch themes and customizing CSS as needed.
Overall, Tailwind and Chakra UI both provide unique advantages — creative freedom and modern theming — 
React-Bootstrap strikes the most effective balance between stability, usability, and developer productivity.
...
Our design direction aimed to capture a sleek, “hacker-style” aesthetic—something that felt sharp and modern without over-complicating the visuals.
...
\subsection{Theme Designs}
Here are some theme designs that we decided on (note: these themes are subject to change as we continue development):
\begin{itemize}
...
If the design that we are looking for isn't available in Bootswatch, we plan to customize the SCSS files provided by Bootswatch to better fit our desired aesthetic.

\subsection{Lighthouse Test}
Lighthouse Testing is an open source tool run by Google that helps developers improve the quality of their web applications.
Lighthouse can be run from Chrome DevTools, from the command line, or as a Node module.

For testing, we plan to use Chrome DevTools to run Lighthouse on our web application. 
Lighthouse runs a series of audits against the web application, and then generates a report on how well the application did.
The report includes scores for performance, accessibility, best practices, SEO, and PWA (Progressive Web App).

\subsection{Lighthouse goals}

Here is how lighthouse will score our web application:
\begin{itemize}
	\item Performance:
		\begin{itemize}
			\item CLS (Cumulative Layout Shift)
			\item TBT (Total Blocking Time)
	...
	\item SEO
\end{itemize}
An example a lighthouse report is shown in figure \ref{fig:lighthouse}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{lighthouse.png}
    \caption{Lighthouse Report}
    \label{fig:lighthouse}
\end{figure}

Our goal is to ensure that all of the scores from lighthouse are above 90.
Our main focus will be on performance and accessibility, as these are the most important factors for a web application.

On top of this, we will also focus on ensuring the best practices, and valid search engine optimization (SEO) techniques are being used.
...
Transparency is an important principle in the design of this application. We strive to minimize the amount of data collected and stored, both to reduce potential vulnerabilities and because we believe it is unethical to retain information that is unnecessary. While certain data must be stored to carry out essential functions, maintain system reliability, and ensure security, we would like to provide all users or potential users with a clear explanation of how their data is used.

To support this goal, the application will include a dedicated Data Usage and Privacy page that will be accessible without requiring a registered account. This page will cover exactly what information is collected, why it is collected, and how long it will be stored. Our intention is to build trust and confidence with any users of WADT by being open and explicit about the platform’s data and security practices.

The Data Usage and Transparency Policy will include the following sections and information:

\subsubsection{Data Collection}
\begin{itemize}
	\item A clear explanation of all data that will be collected and the reasoning behind it
	\item Our commitment to minimal data storage
\end{itemize}

\subsubsection{Data Storage}
\begin{itemize}
	\item Sensitive configuration data is stored exclusively in environment variables
	\item A local .env file is used for development and excluded from version control via .gitignore
	\item Procedures for dealing with credential exposure
	\item The inclusion of regular security audits and automated scanning to maintain credential safety.
	\item Logs and user information will be stored for the minimum time required to support debugging, analytics, or security auditing
\end{itemize}

\subsubsection{Access Permissions}
\begin{itemize}
	\item Explicit descriptions of what instructors can see, what actions they can perform, and how their access differs from that of students.
	\item Application of the principle of least privilege.
\end{itemize}

\subsubsection{Logging and Data Retention}
\begin{itemize}
	\item Detailed information about what container related information is being logged and why logging is necessary.
	\item Clear descriptions of how long logs are retained and when they are deleted.
\end{itemize}

\subsubsection{Commitment to Ethical Data Handling}
\begin{itemize}
	\item Explicit descriptions of what information is intentionally not stored.
	\item A guarantee that no data will be sold, shared, or used for any purposes unrelated to the application.
	\item A commitment that user information will never be tracked for any non-educational purpose.
\end{itemize}

We feel strongly about transparency in all aspects of data storage and handling. There are far too many modern applications and platforms that do not uphold these values. Because WADT is an educational tool, users should feel comfortable and confident while interacting with it. While the primary purpose of the platform is to teach students about application vulnerabilities and security practices, we also hope that our emphasis on transparency will help students understand the importance of ethical data management and responsible system design.
...
\diagram{erd.png}{WADT ERD}{fig:erd}
...
\begin{figure}[H]
    \centering
    \includegraphics[keepaspectratio,height=0.5\textheight,width=\textwidth]{WADT_Use_Case.pdf}
    \caption{WADT Web Application Use Case Diagram}
    \label{fig:usecase}
\end{figure}

\subsection{Deploying Containers}
The activity diagram in figure \ref{fig:deployContainer} details the process behind deployment of a container, from catalog selection through backend processing. 

\begin{figure}[H]
    \centering
    \includegraphics[keepaspectratio,height=0.5\textheight,width=\textwidth]{AD-deploying-containers.png}
    \caption{Container Deployment Activity Diagram}
    \label{fig:deployContainer}
\end{figure}
...
The figure \ref{fig:interactContainers} illustrates the management actions available to users during an active session.

\begin{figure}[H]
    \centering
    \includegraphics[keepaspectratio,height=0.5\textheight,width=\textwidth]{AD-interacting-w-containers.png}
    \caption{Interacting with containers}
    \label{fig:interactContainers}
\end{figure}
...
Figure \ref{fig:cleanup} describes the cleanup process that occurs when a container session ends.

\begin{figure}[H]
    \centering
    \includegraphics[keepaspectratio,height=0.5\textheight,width=\textwidth]{AD-container-cleanup.png}
    \caption{Container Cleanup}
    \label{cleanup}
\end{figure}
...
\subsection{Framework Selection: Django and DRF}

Reasons for choosing Django + DRF:
\begin{itemize}
    \item Built-in ORM and admin panel, make managing database easier with migrations
    \item Django Channels for WebSockets
    \item Function-based views + viewsets make a powerful set of abstractions available
\end{itemize}

We chose a REST API architecture with React instead of Django MVT alone, for
these reasons:
\begin{itemize}
    \item Web-browsable HTML version of API
    \item Viewsets inheriting common CRUD operations
    \item Serializer classes enforce separation of concerns
    \item Backend and frontend are more independent
\end{itemize}

\subsection{Programming Language: Python}

Python was chosen for its Docker SDK integration and compatibility with Django. We opted not to use Go because its Docker SDKs
were low-level and detail-heavy in comparison, beyond what was needed to implement WADT.

\subsection{Containerization Tool: Docker over Podman}

We chose Docker over Podman for the following reasons:
\begin{itemize}
    \item More extensive Python SDK
    \item Better ecosystem and documentation
    \item Highly available public registries (DockerHub and others)
    \item No potential incompatibilities that Podman might include
\end{itemize}

\subsection{Docker SDK: python-on-whales over docker-py}

python-on-whales is a better fit for WADT, as was experimentally verified through the two prototypes.
The following advantages make python-on-whales a more attractive choice:
\begin{itemize}
    \item Thread safety. Guaranteed thread-safe client means views are simpler to write
    \item Compose support. Built-in Docker Compose support extends range of available vulnerable apps
    \item Clear API. python-on-whales mirrors the Docker CLI syntax almost perfectly.
\end{itemize}

\subsection{Database: PostgreSQL}

The team evaluated MongoDB, MySQL, MariaDB, and PostgreSQL, ultimately choosing PostgreSQL for its
support and documentation.
The database runs locally on the AWS EC2 instance. We may migrate to cloud storage if log entries result
in a larger database, requiring a dedicated storage solution.

\subsection{Async Support: Django Channels over Celery}

For real-time status information from containers, we chose Django Channels instead of Celery:
\begin{itemize}
    \item Celery's capacity exceeded project needs
    \item Channels has fewer dependencies
\end{itemize}

\subsection{Frontend: React and TypeScript}

React was selected for its extensive selection of useful components.
The reasons for choosing React include:
\begin{itemize}
    \item Efficiency
    \item Component reusability
    \item Enforce division between frontend and backend work, leading to a cleaner codebase
...
\begin{itemize}
    \item Team familiarity and sponsor familiarity
    \item Already available components
    \item Bootswatch themes (Cyborg, Lux, Zephyr, Pulse) available
\end{itemize}

\subsection{Hosting: AWS}
We selected our hosting for the following reasons:
\begin{itemize}
    \item sponsor-provided hosting option was AWS EC2
    \item Improved familiarity
    \item Needed packages compatible with Ubuntu and available in standard repos
\end{itemize}

\subsection{Security Measures}
\begin{itemize}
	\item IP restrictions (deny-by-default)
	\item Limited network access for containers
	\item Resource limits and uptime limits
	\item Docker configuration best practices
\end{itemize}

\subsection{2FA with pyotp + pass-otp}

The authentication system will use TOTP:
\begin{itemize}
    \item Users must have SSH access to the AWS instance to generate auth codes
    \item \texttt{pyotp} generates one-time codes on the instance
    \item Users store secrets using \texttt{pass} with \texttt{pass-otp} extension on the instance
    \item Therefore, users must have SSH access as a prerequisite to access WADT
\end{itemize}

\subsubsection{Secrets Management}
\begin{itemize}
    \item Pre-commit hooks
    \item Scan every commit
    \item Measures in place for rotation and emergencies relating to leaks
\end{itemize}

\subsection{Vulnerable Application Catalog}

\subsubsection{Intentional Vulnerabilities:}
\begin{itemize}
    ...
    \item cVWA
    \item NIVA
    \item Vulnerableapp-facade
    \item BodgeIt Store
    \item Damn Vulnerable RESTaurant
\end{itemize}

\subsubsection{Unintentional CVEs:}
\begin{itemize}
    \item Ghost CMS (CVE-2023-32235)
    \item PrestaShop (CVE-2023-3882)
    \item Ghost Kit WordPress Plugin (CVE-2025-9992)
\end{itemize}
...
\begin{itemize}
    ...
    \item Maximum uptime (24hrs)
\end{itemize}

\subsection{CI/CD Pipeline}

\begin{enumerate}
    \item TruffleHog scan
    \item Backend tests
    \item Frontend tests
...
\subsection{Product Backlog with User Stories}
\begin{itemize}
	\item arbitrary command execution - Medium - To Do
	\item admin controls/advanced - Medium - To Do
	\item edit network settings per-container - Medium - To Do
	\item admin controls/basic - Medium - To Do - container resource usage limits and auto-down after 24hrs
	\item colorschemes switching - Medium - To Do
	\item test dockerized TVWAs - Highest - To Do - "As a backend developer, I want to ensure that each containerized Target Vulnerable WebApp is functioning properly so that immediate problems can be diagnosed."
	\item Docker Compose compatibility - Medium - To Do - "As a user, I want WADT to work for both single-container and Compose projects, so that I can practice while ignoring Docker-specific details"
	\item accessibility testing - Medium - To Do - "As a user, I want a fully accessible site so that I can use WADT under a diverse set of circumstances"
	\item performance testing - Medium - To Do - "As a sponsor providing hosting, I want a site that has minimal overhead to minimize bandwidth charges."
	\item domain name - Highest - In Review - "As a user, I want a convenient domain to use to access WADT so that I can easily remember it and start practicing."
	\item 2-factor auth - Lowest - To Do - "As a site administrator, I need 2-factor authentication for user logon to prevent unauthorized access from bad actors."
	\item write API spec - High - To Do - "As a frontend developer, I require an API spec in order to develop frontend UI components for the project."
	\item initial deployment - Highest - To Do - "As a user, I want a real website to access so that I don’t have to install software on my computer."
	\item set up CI/CD pipeline - Highest - To Do - "As a developer, I want a CI/CD pipeline to format, lint, test, and scan for vulnerabilities my code before deploying it automatically, so that I don’t have to hurt my wrists typing the same things over and over."
	\item deploy database - Highest - To Do - "As a developer, I need a database on a server to store data so that my app works."
	\item dockerize unintentionally vulnerable webapps 2 - Medium - Done - "As a user, I want WADT to include modern webapps with unintentional vulnerabilities which are published CVEs, so that I can practice in a realistic environment."
	\item dockerize unintentionally vulnerable webapps 1 - Medium - Done - "As a user, I want WADT to include modern webapps with unintentional vulnerabilities which are published CVEs, so that I can practice in a realistic environment."
	\item Dockerize second 5 Web Apps - High - Done
	\item Dockerize first 5 Web Apps - High - Done
	\item Create repo - Highest - Done - "As a developer, I want a repository to work on that will automatically deploy to the instance so that code changes are tracked"
	\item Design Main Page Template - Lowest - In Review - "Communicate with Backend to determine what is necessary for a main page (where the user can access the docker containers). Then use Figma to design a wireframe that matches the theme of the Home page."
	\item Design Login Page Template - Lowest - In Review - Design a wireframe login page utilizing the themes from the home page on figma
	\item Design Home Page Template - Lowest - In Review - "Design a wireframe home page using Figma to determine: * Website Color Theme * Website Format * Other UI components"
	\item Determine essential interactions - High - Done - Answer the question: what do our users want to do with Docker containers? The answer determines which API endpoints the backend must expose and which controls the UI team must anticipate.
	\item Integrate with backend team - Medium - To Do
	\item "create reports for each image (file size -  potential problems -  etc.)" - Medium - To Do
	\item write Dockerfiles/compose files - Medium - To Do
	\item Select 7 apps - Medium - To Do
	\item choose 10 vulnerable web apps and propose to sponsor - Lowest - Done - Select and containerize 7 appropriate vulnerable apps
	\item security audit - Medium - To Do - Entire team verifies the security practices used to set up the AWS instance. Each member documents potential issues with the setup.
	\item docker seminar from sponsor - High - Done - Attend meeting with sponsor where he will show us how to identify and dockerize vulnerable apps.
	\item document security - High - To Do - "Based on security decisions made for sandboxing containers, create documents to explain choices and setup to sponsor, our future selves -  potential users, and the Senior Design Committee."
	\item Prereqs for hosting - Highest - Done
	\item learn to dockerize - Medium - Done - Research the process of Dockerizing a sample web app and perform this task on a demo example.
	\item create UI mockups - Low - Done - create mockups/wireframes for the UI components of each page
	\item setup database - Medium - Done - "Select the database for the project. Create a schema or model of the data. Project scope is one user at a time, limited enough to allow for not putting much thought into the database. However, an approach that at least allows for the possibility of multiple users eliminates technical debt which may haunt future SD teams."
	\item aws instance setup - High - Done - Ensure all members can SSH into the AWS instance. Ensure instance is secure.
	\item sandboxing - Highest - Done - Determine how to sandbox vulnerable applications for safe hosting on an AWS instance. Must protect instance from privilege escalation attacks and prevent internet-connected containers from being used in botnets or similar.
\end{itemize}

\subsection{Sprint Backlog with User Stories}
\begin{itemize}
\item test dockerized TVWAs,"As a backend developer, I want to ensure that each containerized Target Vulnerable WebApp is functioning properly so that immediate problems can be diagnosed."
\item Docker Compose compatibility,"As a user, I want WADT to work for both single-container and Compose projects, so that I can practice while ignoring Docker-specific details"
\item domain name,"As a user, I want a convenient domain to use to access WADT so that I can easily remember it and start practicing."
\item write API spec,"As a frontend developer, I require an API spec in order to develop frontend UI components for the project."
\item initial deployment,"As a user, I want a real website to access so that I don’t have to install software on my computer."
\item set up CI/CD pipeline,"As a developer, I want a CI/CD pipeline to format, lint, test, and scan for vulnerabilities my code before deploying it automatically, so that I don’t have to hurt my wrists typing the same things over and over."
\item deploy database,"As a developer, I need a database on a server to store data so that my app works."
\item Design Main Page Template,"Communicate with Backend to determine what is necessary for a main page (where the user can access the docker containers) Then use Figma to design a wireframe that matches the theme of the Home page."
\item Design Login Page Template,Design a wireframe login page utilizing the themes from the home page on figma
\item Design Home Page Template,"Design a wireframe home page using Figma to determine: * Website Color Theme * Website Format * Other UI components"
\item security audit,Entire team verifies the security practices used to set up the AWS instance. Each member documents potential issues with the setup.
\item document security,"Based on security decisions made for sandboxing containers, create documents to explain choices and setup to sponsor, our future selves, potential users, and the Senior Design Committee."
\end{itemize}

\complexgraphic{sprint-backlog}{Current Sprint Backlog with User Stories}{diagram:backlog}

\subsection{Milestone Chart}
\subsubsection{Current Milestone Chart}
\complexgraphic{timeline}{Current Milestone Chart (End of SD1)}{diagram:timeline}

\subsubsection{Projected Milestone Chart}
\complexgraphic{gantt}{Projected Milestone Chart (End of SD2)}{diagram:future-timeline}

\subsection{Setbacks}
\begin{itemize}
	\item Security research - didn't fit into original timebox. Team fell behind on first spike.
		Adapt through simplifying requirements and communicating with sponsor.
	\item Deployment - delayed due to issues with team members accessing hosting. Slight delay on first sprint.
		Adapt through creating team-internal documentation.
	\item Early Security vulnerability - due to an error in planning early sprints, repository created before security measures fully in place. Django secret keys accidentally leaked due to human error. No delay, but would be problematic if site active at time of leak.
		Adapt through committing to spike of research on security scanning tools and CI/CD pipeline.
\end{itemize}

\subsubsection{Lessons Learned}
Early rapid prototyping and constant communication have been vital for the team.
As we uncover oversights in our original plans, we are able to quickly adapt and modify our sprints.
We are fully equipped to continue the development cycle through SD2 while fulfilling our MVP.

\section{Evaluation}
\begin{figure}[H]
	\centering
	\includegraphics[keepaspectratio,height=0.25\textheight,width=0.5\textwidth]{lighthouse1.png}
	\caption{Lighthouse Scores: Welcome Page}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[keepaspectratio,height=0.25\textheight,width=0.5\textwidth]{lighthouse2.png}
	\caption{Lighthouse Scores: Dashboard Page}
\end{figure}

\subsection*{Checklist}
\begin{itemize}
	\item A user can successfully deploy a target vulnerable webapp using the site
	\item A user's container usage is limited to 4 containers at a time with a maximum lifetime for each container of 24hrs
	\item Restart and stop work correctly
	\item Container status is indicated through the site
\end{itemize}

\section{Analysis}
\begin{figure}[H]
	\centering
	\includegraphics[keepaspectratio,height=0.25\textheight,width=0.5\textwidth]{performance.png}
	\caption{Lighthouse Scores: Performance}
\end{figure}

\section{Continuing On}
The following areas would be most fruitful for new groups of students to continue to improve WADT.
\begin{itemize}
	\item Fully-featured "add container" page. The difficulty of enabling this feature is not
		allowing users to add new containers to the site, as that is fairly easy. All it requires
		is supplying the name of an image. The challenge is in verifying that the supplied image
		is not malicious. This feature requires establishing other complementary features to ensure
		trust of specific users with elevated privileges, and potentially even advanced features
		that can heuristically gauge whether a container is suitable to be added to WADT's catalog
		(As an example, a feature that scans the README page of a suggested image, looking for red flags).
	\item Arbitrary command execution. There are scenarios where a student may want fine-grained control of a
		running container in ways that are not accessible through its UI. They may want to run arbitrary
		commands inside of that container, similar to what experienced command-line users do with \texttt{docker exec}.
		The challenge a new group of students would face is the following: how can they permit command execution
		without allowing for malicious commands? This problem is extremely challenging as there is a huge variety
		of methods to disguise malicious commands to circumvent simplistic detection methods.
	\item Integration with other common pentesting and cybersecurity tools. Can future groups of students make
		it easier for learners to use their existing toolbox to interact with WADT's containers? As an example,
		features may be added that make it easier to use remote debugging tools to step through the execution flow
		inside a container.
\end{itemize}

\section{How to Run WADT}
\subsection{Local Environment}
\begin{enumerate}
	\item Clone repository: \texttt{git clone https://github.com/dallen97/Web-App-Deployment-Tool}
	\item  Set up frontend: follow readme to install dependencies and run development frontend
	\item Set up backend: follow readme to install PostgreSQL, configure database, and install Python dependencies
	\item Ensure Docker compose is installed
	\item Use provided YAML file to run Traefik
	\item Start backend: \texttt{python manage.py runserver}
	\item Start frontend: \texttt{npm run dev}
	\item Start Traefik: \texttt{docker compose -f docker-compose.traefik.yml up -d}
	\item WADT locally now accessible at \texttt{http://localhost:5173}
\end{enumerate}

\section{What is Working, and What Remains Unfinished}
The core functionality of the app is working correctly. Users can perform all basic necessary functions with containers.
Some stretch goals are yet to be implemented. The most significant of these, as of 4/3, is the ability to edit network
settings per-container. This feature has been pushed back since the team opted for exposing containers through different subdomains
(i.e., container1.wadt.com instead of wadt.com:8001) instead of using port numbers, making the ability to change port numbers not
necessary for differentiating between containers.

The stretch goal of allowing site admins to add new containers is currently unfinished due to the complexity and security risk of
allowing users, even if they are authenticated admins, to upload arbitrary Docker compose files or specify arbitrary images for pulling.

Full documentation allowing future groups of students to reproduce our development and testing environments is ongoing.
...
For the purposes of this project, we have not needed to secure any copyright permissions.
